...
def patrol(seen):
    ...
    st, items = api('GET', 'contents/inbox', repo='chepin-ai/qlv')
    if st == 200 and isinstance(items, list):
        for i in items:  # 修SENSE-WINDOW-01: 78件auto-otp积压自此可泄
            if i['name'] != '.gitkeep' and ('line:' + i['name']) not in seen:
                events.append({'kind': 'line-inbox', 'ref': 'qlv:' + i['name']})
    # TOWER-ADOPT: 兼感联邦lane(vci-inbox lanes/vinf/inbox)——LQ/DISC-PROPAGATE类件道
    st, items = api('GET', 'contents/lanes/qlv/inbox', repo='chepin-ai/vci-inbox')
    if st == 200 and isinstance(items, list):
        for i in items:
            if i['name'] != '.gitkeep' and ('lane:' + i['name']) not in seen:
                events.append({'kind': 'lane-inbox', 'ref': 'vci-inbox:lanes/qlv/inbox/' + i['name']})
    return events
...
def kimi_work(events):
    ...
    req = urllib.request.Request('https://api.moonshot.cn/v1/chat/completions',
        method='POST', data=json.dumps({
            ...
            'messages': [
                {'role': 'system', 'content': '你是 qlv 线（第七线·量子律吕·正本chepin-ai/qlv·38件积压泄洪中）无人驿开工分身。读候件，用中文答四件:①何事②与qlv主线何干③应动何件④生债一条。简。'},
...
def main():
    ...
    receipt = {'v': 'QLV-TOWER-01', 'ts': ts, 'idle_in': state.get('idle', 0),
...
# BOARD-VOICE-01 (vinf)
BOARD_INTENT_RE_QLV = __import__('re').compile(r'(板面|post|广播|回应|收讫|对位|认领|开工|成果|异议|报告|判|verdict)', __import__('re').I)

def board_voice_qlv(verdict_memo, parent_ts):
    title = f'qlv-voice-{parent_ts}.md'
    body = f'# qlv 塔声 — {parent_ts}\n\n{verdict_memo[:2000]}\n\n#noauto'
    p = '/tmp/_bv_' + title
